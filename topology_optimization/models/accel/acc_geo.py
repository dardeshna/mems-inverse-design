...
def build_accel(suspension_beam_width, proof_mass_length):
	...
	with pg.occ.geometry.Geometry() as geom:
		...
		geom.save_geometry('accel.step')
		# Be sure to change the gmsh_path to the installed folder
		# meshio.write("accel_tetra.inp", meshio.Mesh(points=mesh.points, cells={'tetra': mesh.cells_dict['tetra']}))

def build_blank(suspension_beam_width, proof_mass_length):
	...
	with pg.occ.geometry.Geometry() as geom:
		...
		# Beam1_1 (long beam)
		d = 2*(suspension_beam_length + beam_dist) - proof_mass_length
		p1 = [0,0,beam_lower]
		p2 = [d, d, beam_thickness]
		beam1 = geom.add_box(p1,p2,char_length=cl*scale)

		#Complete Union
		final = geom.boolean_union([beam1,proof_mass])

		# mesh = geom.generate_mesh(verbose=True)

		geom.save_geometry('accel_blank.step')

		# Be sure to change the gmsh_path to the installed folder
		# meshio.write("accel_blank_tetra.inp", meshio.Mesh(points=mesh.points, cells={'tetra': mesh.cells_dict['tetra']}))


build_blank(suspension_beam_width,proof_mass_length)